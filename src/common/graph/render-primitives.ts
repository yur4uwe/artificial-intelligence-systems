...
export function drawNode(
    ...
    if (tagBadge) {
        const tagColor = state === 'start' ? nodeColors.start.stroke : (state === 'goal' ? nodeColors.goal.stroke : theme.ui.accentPrimary);
        drawTagBadge(ctx, x, y + radius + 11, tagBadge, tagColor);
    } else if (state === 'start') {
        drawTagBadge(ctx, x, y + radius + 11, 'START (S)', nodeColors.start.stroke);
    } else if (state === 'goal') {
        drawTagBadge(ctx, x, y + radius + 11, 'GOAL (G)', nodeColors.goal.stroke);
    }
}
...
export function createNodeDrawer(
    ...
    return (node: GraphNode | NodeDrawOptions): void => {
        ...
        const isHovered = isGraphNode ? hoveredNodeId === node.id : !!node.isHovered;
        const isSelected = isGraphNode ? selectedNodeId === node.id : !!node.isSelected;
...
export function createEdgeDrawer(
    ctx: CanvasRenderingContext2D,
    theme: ThemePalette,
    getNode: (id: number) => { x: number; y: number; radius?: number } | undefined
): (edge: GraphEdge | EdgeDrawOptions) => void {
    return (edge: GraphEdge | EdgeDrawOptions): void => {
        if ('from' in edge && 'to' in edge && edge.from !== undefined && edge.to !== undefined) {
