      ...
      <div class="flex flex-col gap-4 text-xs">
        ...
        <div class="bg-slate-900 border border-slate-800 rounded-xl p-3 shadow-md">
          ...
          <div class="bg-slate-950/80 rounded-lg p-2.5 border border-slate-800/80">
            ...
            <div id="mp-path-display" class="font-mono text-xs text-indigo-300 break-words font-medium min-h-[1.5rem]">
              —
            </div>
          </div>
        </div>
        ...
        <div class="grid grid-cols-2 gap-2.5">
          ...
          <div class="bg-slate-900 border border-slate-800 rounded-xl p-2.5">
            ...
            <span id="mp-exec-time" class="text-lg font-bold text-indigo-400 font-mono">0.00</span>
        ...
        <div class="bg-slate-900 border border-slate-800 rounded-xl p-3 shadow-md">
          <div class="flex items-center justify-between mb-2">
            <span class="text-slate-300 font-semibold flex items-center gap-1.5">
              <span>📋 Черга вершин (FIFO Queue)</span>
            </span>
  ...
  public updateStep(event: StepEvent): void {
    ...
    if (event.foundPath && event.foundPath.length > 0) {
      this.pathDisplay.innerHTML = event.foundPath
        .map((id, i) => `<span class="text-indigo-300 font-bold">${id}</span>${i < event.foundPath!.length - 1 ? ' <span class="text-slate-400">→</span> ' : ''}`)
        .join('');
    ...
    if (event.queue.length === 0) {
      ...
      this.queueContainer.innerHTML = event.queue
        .map(id => `
          <span class="inline-flex items-center px-2 py-0.5 rounded-md text-[11px] font-mono font-medium bg-cyan-950 text-cyan-300 border border-cyan-700/50 shadow-sm">
            v${id}
          </span>
        `)
        .join('');
    ...
    if (event.actionDescription) {
      ...
      logItem.innerHTML = `
        <span class="text-indigo-400 font-bold shrink-0">#${event.stepIndex}</span>
