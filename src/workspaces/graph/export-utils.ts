...
export function exportMetricsToCSV(
    ...
    lines.push(
        `"Знайдений шлях","${metrics.foundPath ? metrics.foundPath.join(' -> ') : 'Не знайдено'}"`
    )
    ...
    lines.push('')
    lines.push(
        '"Крок","Поточна вершина","Черга (Queue)","Розкрито","Дія / Пояснення"'
    )

    traceHistory.forEach((step, idx) => {
        const queueStr = `[${step.frontier.join(', ')}]`
        const actionClean = step.actionDescription.replace(/"/g, '""')
        lines.push(
            `${idx + 1},${step.currentNodeId ?? '-'},"${queueStr}",${step.openedCount},"${actionClean}"`
        )
    })
