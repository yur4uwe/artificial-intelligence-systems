...
export type NeighborSortingStrategy =
    'ascending-id' | 'descending-id' | 'clockwise' | 'custom'
...
export class GraphModel {
    ...
    constructor(
        ...
        constraints: GraphConstraints = DEFAULT_GRAPH_CONSTRAINTS
    ) {
    ...
    public removeNode(id: number): void {
        ...
        this.edgesList = this.edgesList.filter(
            (e) => e.from !== id && e.to !== id
        )
        this.version++
    }
    ...
    public hasPath(
        ...
        while (queue.length > 0) {
            ...
            for (const neighbor of neighbors) {
                if (isDirected) {
                    const edge = this.edgesList.find(
                        (e) =>
                            e.from === curr && e.to === neighbor && e.isDirected
                    )
                    if (!edge) continue
                }
    ...
    public canAddEdge(
        ...
        if (from === to && !this.constraints.allowSelfLoops) {
            return {
                valid: false,
                reason: 'Петлі (ребра вершини до самої себе) заборонені',
            }
        }
        ...
        if (isDirected && !this.constraints.allowDirectedEdges) {
            return {
                valid: false,
                reason: 'Орієнтовані дуги заборонені для цього типу графа',
            }
        }
        ...
        if (!isDirected && !this.constraints.allowUndirectedEdges) {
            return {
                valid: false,
                reason: 'Неорієнтовані ребра заборонені для орієнтованого графа',
            }
        }

        // Check if edge already exists
        const existing = this.edgesList.find(
            (e) =>
                (e.from === from && e.to === to) ||
        ...
        // Max degree check
        const fromDegree = this.edgesList.filter(
            (e) => e.from === from || (!e.isDirected && e.to === from)
        ).length
        const toDegree = this.edgesList.filter(
            (e) => e.to === to || (!e.isDirected && e.from === to)
        ).length
        if (
            fromDegree >= this.constraints.maxEdgesPerNode ||
            toDegree >= this.constraints.maxEdgesPerNode
        ) {
            return {
                valid: false,
    ...
    public addEdge(
        ...
        if (!validation.valid) {
            alert(`[GraphModel] addEdge rejected: ${validation.reason}`)
            return null
        }
    ...
    public getNeighbors(
        nodeId: number,
        strategy: NeighborSortingStrategy = 'ascending-id'
    ): number[] {
    ...
    public resetVisualStates(
        keepStartAndGoal: { startId: number | null; goalId: number | null } = {
            startId: null,
            goalId: null,
        }
    ): void {
