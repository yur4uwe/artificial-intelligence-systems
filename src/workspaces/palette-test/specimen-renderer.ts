...
import { createNodeDrawer, createEdgeDrawer, drawEdge } from '@common/graph/render-primitives';
...
export class PaletteSpecimensRenderer {
    ...
    public static renderMiniGraph(canvas: HTMLCanvasElement, theme: ThemePalette): void {
        ...
        const nodes = [
            { id: 1, label: 'S', x: 75 * scaleX, y: 150, state: 'start', tagBadge: 'START (S)', visitIndex: 1 },
            { id: 2, label: 'v2', x: 200 * scaleX, y: 80, state: 'path', visitIndex: 2 },
            { id: 3, label: 'v3', x: 200 * scaleX, y: 220, state: 'visited', visitIndex: 3 },
            ...
            { id: 9, label: 'G', x: 720 * scaleX, y: 150, state: 'goal', tagBadge: 'GOAL (G)' },
        ...
        const edges: GraphEdge[] = [
            ...
            // Idle / Cross links
            { id: 'e9', from: 2, to: 4, state: 'idle', isDirected: true, weight: 8 },
        ...
        const renderEdge = createEdgeDrawer(ctx, theme, id => nodes.find(n => n.id === id));
