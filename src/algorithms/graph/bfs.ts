...
export class BFSAlgorithm implements RunnableAlgorithm<GraphStepEvent> {
    ...
    /**
     * Synchronous, non-yielding pure BFS execution for fast execution/benchmarking.
     */
    public runPure(): {
    ...
    /**
     * Runs a micro-benchmark with JIT warm-up and caches the result on this instance.
     */
    ...
    private *generateSteps(): Generator<GraphStepEvent, void, unknown> {
        ...
        while (queue.length > 0) {
            ...
            // Get sorted neighbors according to selected strategy
            const neighbors = model.getNeighbors(currentId, sortingStrategy)
            const unvisitedNeighbors = neighbors.filter((n) => !visited.has(n))
