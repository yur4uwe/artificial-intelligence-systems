    ...
    this.container.innerHTML = `
      <div class="flex flex-col gap-4 text-xs">
        ...
        <div class="bg-slate-900 border border-slate-800 rounded-xl p-3 shadow-md">
          ...
          <select id="l1-preset-select" class="w-full bg-slate-950 border border-slate-700 rounded-lg px-2.5 py-1.5 text-xs text-slate-100 font-medium focus:ring-1 focus:ring-indigo-500 outline-none">
        ...
        <div class="bg-slate-900 border border-slate-800 rounded-xl p-3 shadow-md">
          <div class="flex items-center justify-between mb-2">
            ...
            <button id="l1-btn-swap" title="Дзеркальна заміна Start ↔ Goal (Вимога лаби)" class="flex items-center gap-1 px-2 py-1 rounded bg-slate-800 hover:bg-slate-700 text-indigo-300 border border-slate-700 font-medium transition active:scale-95">
              <span>⇄</span>
              <span>Поміняти</span>
            </button>
          </div>
        ...
        <div class="bg-slate-900 border border-slate-800 rounded-xl p-3 shadow-md">
          ...
          <div class="flex flex-col gap-1.5 text-[11px] text-slate-300">
            <label class="flex items-center gap-2 cursor-pointer p-1 rounded hover:bg-slate-800/50">
              <input type="radio" name="neighbor-order" value="ascending-id" checked class="accent-indigo-500">
              <span>За зростанням ID (<code class="text-indigo-400 font-mono">1 → 2 → 3...</code>)</span>
            </label>
            <label class="flex items-center gap-2 cursor-pointer p-1 rounded hover:bg-slate-800/50">
              <input type="radio" name="neighbor-order" value="descending-id" class="accent-indigo-500">
              <span>За спаданням ID (<code class="text-indigo-400 font-mono">...3 → 2 → 1</code>)</span>
            </label>
            <label class="flex items-center gap-2 cursor-pointer p-1 rounded hover:bg-slate-800/50">
              <input type="radio" name="neighbor-order" value="clockwise" class="accent-indigo-500">
        ...
        <div class="bg-slate-900 border border-slate-800 rounded-xl p-3 shadow-md">
          <div class="flex items-center justify-between mb-2">
            <span class="text-slate-300 font-semibold">🛠️ Редагування графа на полотні:</span>
            <div class="flex gap-1">
              <button id="l1-btn-fit" title="Підігнати граф під розмір екрану" class="p-1 px-1.5 rounded text-[10px] bg-slate-800 hover:bg-slate-700 text-slate-300 border border-slate-700 transition">
                🔍 Авто
              </button>
              <button id="l1-btn-reset-view" title="Скинути зум/панораму" class="p-1 px-1.5 rounded text-[10px] bg-slate-800 hover:bg-slate-700 text-slate-300 border border-slate-700 transition">
                1:1
              </button>
            </div>
          </div>
          ...
          <div class="grid grid-cols-2 gap-1.5 text-[11px]">
            <label class="flex items-center gap-1.5 p-1.5 rounded bg-slate-950 border border-slate-800 hover:border-slate-700 cursor-pointer">
              <input type="radio" name="canvas-mode" value="select" checked class="accent-indigo-500">
              <span>👆 Рухати / Вибір</span>
            </label>
            <label class="flex items-center gap-1.5 p-1.5 rounded bg-slate-950 border border-slate-800 hover:border-slate-700 cursor-pointer">
              <input type="radio" name="canvas-mode" value="add-node" class="accent-indigo-500">
              <span>➕ Вершина</span>
            </label>
            <label class="flex items-center gap-1.5 p-1.5 rounded bg-slate-950 border border-slate-800 hover:border-slate-700 cursor-pointer">
              <input type="radio" name="canvas-mode" value="add-edge-undirected" class="accent-indigo-500">
              <span>🔗 Ребро</span>
            </label>
            <label class="flex items-center gap-1.5 p-1.5 rounded bg-slate-950 border border-slate-800 hover:border-slate-700 cursor-pointer">
              <input type="radio" name="canvas-mode" value="add-edge-directed" class="accent-indigo-500">
              <span>➡️ Дуга (оргграф)</span>
            </label>
            <label class="flex items-center gap-1.5 p-1.5 rounded bg-slate-950 border border-slate-800 hover:border-slate-700 cursor-pointer text-emerald-300">
              <input type="radio" name="canvas-mode" value="set-start" class="accent-emerald-500">
              <span>🟢 Обрати Start</span>
            </label>
            <label class="flex items-center gap-1.5 p-1.5 rounded bg-slate-950 border border-slate-800 hover:border-slate-700 cursor-pointer text-rose-300">
              <input type="radio" name="canvas-mode" value="set-goal" class="accent-rose-500">
              <span>🔴 Обрати Goal</span>
            </label>
            <label class="flex items-center gap-1.5 p-1.5 rounded bg-slate-950 border border-slate-800 hover:border-slate-700 cursor-pointer text-rose-400 col-span-2">
              <input type="radio" name="canvas-mode" value="delete" class="accent-rose-500">
              <span>🗑️ Видалити елемент (клік на вершину/ребро)</span>
            </label>
          </div>
        </div>
      </div>
