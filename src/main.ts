...
class App {
    ...
    this.tabsNav.innerHTML = LABS_REGISTRY.map(lab => `
      <button 
        ...
        class="lab-tab-btn px-3 py-1.5 rounded-lg text-xs font-medium transition flex items-center gap-2 ${
          lab.id === this.activeLabId 
            ? 'bg-indigo-600 text-white shadow-md shadow-indigo-600/30' 
            : 'text-slate-400 hover:text-slate-200 hover:bg-slate-900'
        }"
      >
        <span>${lab.shortTitle}</span>
        <span class="text-[10px] px-1.5 py-0.2 rounded bg-slate-900/60 font-semibold text-slate-300">
          ${lab.badge}
        </span>
      </button>
    `).join('');
  ...
  private updateTabStyles(): void {
    this.tabsNav.querySelectorAll('.lab-tab-btn').forEach(btn => {
      ...
      if (id === this.activeLabId) {
        btn.className = 'lab-tab-btn px-3 py-1.5 rounded-lg text-xs font-semibold transition flex items-center gap-2 bg-indigo-600 text-white shadow-md shadow-indigo-600/30';
      } else {
  ...
  private attachHeaderEvents(): void {
    document.querySelector('#btn-export-screenshot')?.addEventListener('click', () => {
